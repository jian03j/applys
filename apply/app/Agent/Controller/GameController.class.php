...
//        $where="(g.promote_switch = 2) AND (g.is_delete = 2) AND (g.status = 2) AND (gr.benefit_type IS NOT NULL) "
...
//                . "AND (gv.packageurl IS NOT NULL) ";
         ...
         $where="(g.promote_switch = 2) AND (g.is_delete = 2) AND (g.status = 2) "
                . "AND (g.id NOT IN (select app_id from ".C('DB_PREFIX')."agent_game where agent_id =$agent_id)) "
                . " ";
         $where .= ' and g.id in (60041,60054,60005,60200,60056,60119)';
...
//        }
    ...
    public function mygames(){
        ...
        $show = $Page->show();// 分页显示输出  
        $games=$model
                ->field("ag.*,g.name as gamename,g.initial,agr.agent_rate")
                ->alias('ag')
                ->join("LEFT JOIN ".C('DB_PREFIX')."game g ON g.id=ag.app_id" )
        ...
        foreach($games as $key=>$value){            
            $remote_app_fp=DOWNSITE."/sdkgame/".$value['initial']."/".$value['agentgame'].".apk";           
