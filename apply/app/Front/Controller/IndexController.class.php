...
//        echo md5("dj123456"."M@x^@!e@($");
    ...
    public function getServerList($start=0,$limit=0) {
        $model=M('web_server');
        $items=$model
                ->field("ws.*,g.name as game_name,FROM_UNIXTIME(ws.start_time,'%Y-%m-%d %H:%i:%s') as start_time")
                ->alias("ws")
                ->join("LEFT JOIN ".C("DB_PREFIX")."game g ON g.id=ws.app_id")
                //->where(array("ws.is_delete"=>2,"ws.start_time"=>array("egt",time())))
                ->where(array("ws.is_delete"=>2))
                ->limit($start,$limit)
...
//        echo md5("dj123456"."M@x^@!e@($");
        ...
        $hs_obj=new \Huosdk\Game();
        $items=$hs_obj->get_game_list2(0,30);   
...
//       
	$server_items = array_slice($server_items,0,22); 
...
//        $server_paging=$server_page->show();
        $this->ajaxReturn(array("error" => "0", 
            "msg" => json_encode(array("items"=>$items_txt,"max_p"=>$max_p)
            )));
    }
    /* 首页  */
